...
func (r *Reader) Stop() error {
	...
	r.closeOnce.Do(func() {
		...
		if !r.systemd.IsSystemd() {
			if cleanupErr := CleanupSocket(r.socketPath); cleanupErr != nil && err == nil {
				err = cleanupErr
			}
		}

		if systemdErr := r.systemd.Close(); systemdErr != nil && err == nil {
			err = systemdErr
		}
	})
	return err
...
func (r *Reader) run() {
	...
	consecutiveErrors := 0
	for {
		// Set a short deadline for Accept() to allow checking shutdown signal.
		r.listener.(*net.UnixListener).SetDeadline(time.Now().Add(time.Second)) // nolint: errcheck,gosec // This is a known-safe type assertion
		...
		if err != nil {
			...
			default:
				...
				// Exponential backoff to the maximum configured backoff.
				backoff := min(r.initialBackoff<<(consecutiveErrors-1), r.maxBackoff)
				time.Sleep(backoff)
			}
