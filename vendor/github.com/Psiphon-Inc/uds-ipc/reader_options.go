...
// WithBackoffConfig sets the exponential backoff configuration for accept errors.
// initialBackoff is the delay after the first error, which doubles with each
// subsequent error up to maxBackoff. initialBackoff must be <= maxBackoff.
// Default is 100ms initial, 10s maximum.
func WithBackoffConfig(initialBackoff, maxBackoff time.Duration) ReaderOption {
	return func(r *Reader) error {
		if initialBackoff <= 0 {
			return errors.Join(ErrInvalidTimeout, errors.New("initial backoff must be > 0"))
		}
		if maxBackoff <= 0 {
			return errors.Join(ErrInvalidTimeout, errors.New("maximum backoff must be > 0"))
		}
		if initialBackoff > maxBackoff {
			return errors.Join(ErrInvalidTimeout, errors.New("initialBackoff must be <= maxBackoff"))
		}
