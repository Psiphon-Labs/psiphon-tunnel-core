...
//		udsipc.WithBackoffConfig(                     // backoff configuration
//			10*time.Millisecond,  // initial backoff
...
//		udsipc.WithBufferSize(1000),                    // buffer size
//	)
//	if err != nil {
//		log.Fatal(err)
//	}
...
//   - WithBackoffConfig(initial, max time.Duration): Set exponential backoff (default: 100ms, 10s)
//   - WithReadBufferSize(size uint32): Set socket read buffer size (default: 256KB)
//
// Writer options:
//
//   - WithBufferSize(size uint32): Set message buffer size (default: 10,000)
