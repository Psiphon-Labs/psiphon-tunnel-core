...
- (NSString * _Nullable)getDSLAccessToken:(NSError * _Nullable * _Nonnull)error {
    return GoPsiGetDSLAccessToken(error);
}
...
- (void)handlePsiphonNotice:(NSString * _Nonnull)noticeJSON {
    ...
    if ([noticeType isEqualToString:@"Tunnels"]) {
        ...
        if ([self.tunneledAppDelegate respondsToSelector:@selector(onDSLAccessTokenAvailable)]) {
            dispatch_sync(self->callbackQueue, ^{
                [self.tunneledAppDelegate onDSLAccessTokenAvailable];
            });
        }
    }
...
+ (NSString * _Nonnull)getDeviceRegion {
    /// One of the ways we determine the device region is to look at the current timezone. We then need to map that to a likely country.
    /// These overrides align the retained legacy mapping with IANA tzdb 2026c zone.tab:
    /// https://data.iana.org/time-zones/data/zone.tab
    const NSDictionary *ianaTimezoneToCountryCodeOverrides = @{
    ...
    /// Legacy timezone identifiers are retained for older OS versions.
    ...
    NSString *timezoneCountryCode = ianaTimezoneToCountryCodeOverrides[timezone];
    if (timezoneCountryCode == nil) {
        timezoneCountryCode = timezoneToCountryCode[timezone];
    }
    if (timezoneCountryCode != nil) {
