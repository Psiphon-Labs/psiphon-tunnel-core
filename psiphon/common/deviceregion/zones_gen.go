...
const (
	...
	// The MobileLibrary/iOS implementation of getDeviceRegion keeps its own
	// copy of the IANA table. Generating both from here is what keeps them from
	// drifting apart; see the note in the generated header.
	//
	// Paths are relative to this package, which is where go generate runs.
	objcClassName  = "IANATimeZoneCountry"
	objcOutputDir  = "../../../MobileLibrary/iOS/PsiphonTunnel/PsiphonTunnel/Utils"
	objcHeaderName = objcClassName + ".h"
	objcSourceName = objcClassName + ".m"
)
...
func main() {
	...
	writeObjC(ianaZoneCountry, tzdbVersion, windowsZonesDigest, timezoneDigest)
}

// writeObjC emits the Objective-C counterpart of ianaZoneCountry for
// MobileLibrary/iOS. Only the IANA table is emitted: the iOS implementation
// reads NSTimeZone names and has no use for the Windows table.
func writeObjC(ianaZoneCountry map[string]string, tzdbVersion, windowsZonesDigest, timezoneDigest string) {

	zones := make([]string, 0, len(ianaZoneCountry))
	...
	header.WriteString(gplHeader)
	fmt.Fprintf(&header, objcGeneratedNotice, tzdbVersion, tzdbZoneTabURL, tzdbBackwardURL,
		cldrWindowsZonesURL, windowsZonesDigest, cldrTimezoneURL, timezoneDigest)
	fmt.Fprintf(&header, objcHeaderBody, objcClassName)

	var source strings.Builder
	source.WriteString(gplHeader)
	fmt.Fprintf(&source, objcGeneratedNotice, tzdbVersion, tzdbZoneTabURL, tzdbBackwardURL,
		cldrWindowsZonesURL, windowsZonesDigest, cldrTimezoneURL, timezoneDigest)
	fmt.Fprintf(&source, objcSourcePrologue, objcHeaderName, objcClassName)
	for _, zone := range zones {
		fmt.Fprintf(&source, "            @%q: @%q,\n", zone, ianaZoneCountry[zone])
	}
	source.WriteString(objcSourceEpilogue)

	for name, content := range map[string]string{
		objcHeaderName: header.String(),
		objcSourceName: source.String(),
	} {
		path := filepath.Join(objcOutputDir, name)
		if err := os.WriteFile(path, []byte(content), 0644); err != nil {
			log.Fatalf("write %s: %v", path, err)
		}
	}

	log.Printf("wrote %s and %s in %s: %d IANA entries",
		objcHeaderName, objcSourceName, objcOutputDir, len(ianaZoneCountry))
}
...
// objcGeneratedNotice takes the tzdb version, the two tzdb URLs, the CLDR
// digest, and the CLDR URL.
const objcGeneratedNotice = `// Code generated by psiphon/common/deviceregion/zones_gen.go. DO NOT EDIT.
//
...
// objcHeaderBody takes the class name.
const objcHeaderBody = `#import <Foundation/Foundation.h>

NS_ASSUME_NONNULL_BEGIN

/// %[1]s maps IANA time zone names to the countries containing them.
@interface %[1]s : NSObject
...
// objcSourcePrologue takes the header filename and the class name.
const objcSourcePrologue = `#import "%[1]s"
