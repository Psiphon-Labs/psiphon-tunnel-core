...
// TestSNIFragmentOffsetUTLSClientHello checks that sniFragmentOffset locates the
// SNI hostname in a ClientHello marshaled by uTLS, for every TLS profile this
// package can select.
func TestSNIFragmentOffsetUTLSClientHello(t *testing.T) {
	...
	profiles = append(profiles, p.CustomTLSProfileNames()...)

	for _, tlsProfile := range profiles {

		// Randomized profiles produce a different ClientHello per seed, so
		// exercise a range of seeds rather than a single one.
		repeats := 1
		...
		for i := 0; i < repeats; i++ {
			t.Run(fmt.Sprintf("%s/%d", tlsProfile, i), func(t *testing.T) {

				clientHello := buildTestUTLSClientHello(t, p, tlsProfile, serverName)

				offset, err := clientHelloSNIFragmentOffset(clientHello)
				if err != nil {
					t.Fatalf("clientHelloSNIFragmentOffset failed: %v", err)
				}
				if !bytes.HasPrefix(clientHello[offset:], []byte(serverName)) {
					t.Fatalf("fragment does not start at SNI hostname: offset %d in %x",
						offset, clientHello)
				}
			})
		}
...
// buildTestUTLSClientHello builds a ClientHello using the same uTLS
// ClientHelloID/spec selection and utls.Config fields as Dial, without
// requiring a listener.
func buildTestUTLSClientHello(
	t *testing.T,
	p parameters.ParametersAccessor,
	tlsProfile string,
	serverName string) []byte {

	t.Helper()

	utlsClientHelloID, utlsClientHelloSpec, err := getUTLSClientHelloID(p, tlsProfile)
	if err != nil {
	...
	if protocol.TLSProfileIsRandomized(tlsProfile) {
		...
		*utlsClientHelloID.Seed = [32]byte(*seed)
	...
	// Matches the utls.Config in Dial. OmitEmptyPsk is required: profiles
	// carrying a PreSharedKeyExtension, such as TLS_PROFILE_CHROME_112_PSK, fail
	// BuildHandshakeState with ErrEmptyPsk when there is no cached session.
...
func corruptTestClientHelloSNINameLength(clientHello []byte) []byte {
	corrupt := append([]byte{}, clientHello...)
	// Derive the offset from makeTestClientHello's fixed layout rather than
	// from sniFragmentOffset, which is the function under test. Fixed fields:
	// handshake header, version, random, empty session ID, one cipher suite,
	// one compression method, extensions length, SNI extension type and length,
	// server name list length, name type. The next two bytes are the name
	// length; increment its low byte to overrun the name list.
	nameLengthOffset := 4 + 2 + 32 + 1 + 2 + 2 + 1 + 1 + 2 + 2 + 2 + 2 + 1
	corrupt[nameLengthOffset+1]++
	return corrupt
}
