...
// Package portmapper is a UDP port mapping client. It currently allows for
// mapping over NAT-PMP, UPnP, and PCP.
//
// This package is a hard fork of Tailscale's net/portmapper, copied from
// tailscale v1.98.5:
//
//	https://github.com/tailscale/tailscale/tree/v1.98.5/net/portmapper
//
// The fork exists so that Psiphon can:
//
//   - Bind the port mapping client's UPnP/NAT-PMP/PCP discovery and control
//     sockets to a caller-chosen network interface on every platform, via a
//     per-Client BindToDevice hook (see Config.BindToDevice), rather than
//     Tailscale's process-global tailscale.com/net/netns hooks (e.g.
//     netns.SetAndroidProtectFunc). This is required for split-interface
//     proxies that bind upstream and downstream ports to different devices.
//
//   - Probe the network for port mapping services once and then create many
//     independent port mappings that reuse the probe result, via Client.Clone,
//     rather than re-probing for every mapping.
//
//   - Unlink the code from Tailscale-internal boilerplate dependencies
//     (netns, netmon, eventbus, clientmetric, logger, and others), copying
...
type Client struct {
	...
	// control, if non-nil, is set as the Control hook on the net.ListenConfig
	// and net.Dialer used for all port mapping sockets (UPnP/NAT-PMP/PCP
	// discovery and control, and the UPnP SOAP HTTP client). It is built from
	// Config.BindToDevice by bindToDeviceControl and is immutable after
	// construction.
	...
	uPnPHTTPClient *http.Client        // Control-configured HTTP client for UPnP; nil until needed
...
type Config struct {
	// Logf is called to generate text logs for the client. If nil, logs are
	// discarded.
	Logf logf
	...
	// interface configurations); see psiphon/common/portmapper/gateway.
	GatewayLookupFunc func() (gw, myIP netip.Addr, ok bool)

	// BindToDevice, if non-nil, binds every socket the port mapper opens (the
	// UPnP/NAT-PMP/PCP discovery and control sockets, and the UPnP SOAP HTTP
	// client) to a specific network interface, given the socket's file
	// descriptor. It is the per-Client replacement for Tailscale's
	// process-global tailscale.com/net/netns hooks (e.g.
	// netns.SetAndroidProtectFunc).
	//
	// The same hook works on every platform; the caller performs the
	// platform-specific bind (e.g. Android VpnService.protect on Android,
	// SO_BINDTODEVICE on Linux, IP_UNICAST_IF on Windows, IP_BOUND_IF on
	// Darwin). This is the seam that lets split-interface proxies bind the
	// port mapper to a chosen (e.g. downstream/ICE) interface.
	BindToDevice func(fd int) error
...
func NewClient(c Config) *Client {
	ret := &Client{
		...
		ipAndGateway: c.GatewayLookupFunc,
		onChange:     c.OnChange,
	}
	if ret.logf == nil {
		ret.logf = discardLogf
	}
	if c.DebugKnobs != nil {
...
// bindToDeviceControl adapts a BindToDevice(fd) hook into a net.ListenConfig /
// net.Dialer Control function. It returns nil if bind is nil, leaving sockets
// unbound (normal routing).
func bindToDeviceControl(bind func(fd int) error) func(network, address string, c syscall.RawConn) error {
	if bind == nil {
		return nil
	}
...
// SetGatewayLookupFunc sets the func that returns the machine's gateway IP and
// the primary IP address for that gateway, overriding Config.GatewayLookupFunc.
// It must be called before the client is used.
func (c *Client) SetGatewayLookupFunc(f func() (gw, myIP netip.Addr, ok bool)) {
	c.ipAndGateway = f
}

// Clone returns a new Client that reuses the port mapping service discovery
// results (the data gathered by Probe) from c. This lets a caller probe the
// network once and then create multiple independent port mappings without
// re-probing for each one.
//
// The returned Client has no active mapping of its own. It inherits c's logf,
// Control (BindToDevice) hook, gateway lookup, and debug knobs, and uses the
// supplied onChange callback (which may be nil). The caller should set the
// local port with SetLocalPort before creating a mapping.
//
// c must have completed a Probe and be otherwise idle when Clone is called;
...
func (c *Client) Clone(onChange func()) *Client {
	...
	return &Client{
		...
		pmpPubIP:     c.pmpPubIP,
		pmpPubIPTime: c.pmpPubIPTime,
		pmpLastEpoch: c.pmpLastEpoch,
		pcpSawTime:   c.pcpSawTime,
		pcpLastEpoch: c.pcpLastEpoch,
		uPnPSawTime:  c.uPnPSawTime,
		uPnPMetas:    c.uPnPMetas,
	}
}
...
func (c *Client) listenPacket(ctx context.Context, network, addr string) (packetConn, error) {
	// When running under testing conditions, we bind the IGD server to
	// localhost, so we must not apply the Control (BindToDevice) hook: binding
	// a loopback socket to the LAN interface would send packets for 127.0.0.1
	// out the machine's LAN interface, which gets dropped on the floor.
	lc := net.ListenConfig{}
...
func (c *Client) sawUPnPRecently() bool {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.sawUPnPRecentlyLocked()
}

func (c *Client) sawUPnPRecentlyLocked() bool {
	return c.uPnPSawTime.After(time.Now().Add(-trustServiceStillAvailableDuration))
}

// RespondingPortMappingTypes reports which port mapping protocols have
// responded recently (within trustServiceStillAvailableDuration) for this
// Client's current network, based on the discovery state gathered by Probe and
...
func (c *Client) RespondingPortMappingTypes() (upnp, pmp, pcp bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.sawUPnPRecentlyLocked(), c.sawPMPRecentlyLocked(), c.sawPCPRecentlyLocked()
}
...
func parsePMPResponse(pkt []byte) (res pmpResponse, ok bool) {
	...
	if res.OpCode == pmpOpReply|pmpOpMapPublicAddr {
		...
		res.PublicAddr = netaddrIPv4(pkt[8], pkt[9], pkt[10], pkt[11])
...
func (c *Client) Probe(ctx context.Context) (res ProbeResult, err error) {
	...
	upnpMulticastAddr := netip.AddrPortFrom(netaddrIPv4(239, 255, 255, 250), c.upnpPort())
	...
	for {
		...
		switch port {
		case c.upnpPort():
			if bytes.Contains(buf[:n], []byte(":InternetGatewayDevice:")) {
				handleUPnPResponse()
			}
...
var (
	...
	metricUPnPSelectMultiple = newCounter("portmap_upnp_select_multiple")
	...
	metricUPnPSelectExternalPublic = newCounter("portmap_upnp_select_external_public")
	...
	metricUPnPSelectExternalPrivate = newCounter("portmap_upnp_select_external_private")
