...
//
//	https://github.com/tailscale/tailscale/blob/v1.98.5/net/portmapper/igd_test.go
//
...
func NewTestIGD(tb testing.TB, t TestIGDOptions) (*TestIGD, error) {
	logf := whileTestRunningLogf(tb)
	d := &TestIGD{
		doPMP:  t.PMP,
		doPCP:  t.PCP,
		doUPnP: t.UPnP,
...
func testIPAndGateway() (gw, ip netip.Addr, ok bool) {
	return netaddrIPv4(127, 0, 0, 1), netaddrIPv4(1, 2, 3, 4), true
}
...
func (d *TestIGD) servePxP() {
	...
	for {
		...
		src := netaddrUnmap(a.(*net.UDPAddr).AddrPort())
		if !src.IsValid() {
			panic("bogus addr")
		}
		pkt := buf[:n]
...
func newTestClient(t *testing.T, igd *TestIGD) *Client {
	var c *Client
	c = NewClient(Config{
		Logf: whileTestRunningLogf(t),
		OnChange: func() {
			t.Logf("port map changed; have mapping: %v", c.HaveMapping())
		},
	})
