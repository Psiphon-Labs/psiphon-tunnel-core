...
//
//	https://github.com/tailscale/tailscale/blob/v1.98.5/net/portmapper/upnp.go
//
...
var upnpHTTPClientKey = newCtxKey[*http.Client]("portmapper.upnpHTTPClient", nil)
...
func getUPnPRootDevice(ctx context.Context, logf logf, debug DebugKnobs, gw netip.Addr, meta uPnPDiscoResponse) (rootDev *goupnp.RootDevice, loc *url.URL, err error) {
...
func selectBestService(ctx context.Context, logf logf, root *goupnp.RootDevice, loc *url.URL) (client upnpClient, err error) {
	method := "none"
	defer func() {
		if client == nil {
			return
	...
	for _, svc := range clients {
		...
		makSet(&externalIPs, svc, externalIP)
...
func serviceIsConnected(ctx context.Context, logf logf, svc upnpClient) bool {
	...
	if c.uPnPHTTPClient == nil {
		c.uPnPHTTPClient = &http.Client{
			Transport: &http.Transport{
				DialContext:     (&net.Dialer{Control: c.control}).DialContext,
...
func requestLogger(logf logf, client *http.Client) *http.Client {
