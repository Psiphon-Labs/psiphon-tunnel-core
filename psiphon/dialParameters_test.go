...
func runDialParametersAndReplay(t *testing.T, tunnelProtocol string) {
	...
	dialParams, err := MakeDialParameters(
		clientConfig, steeringIPCache, upstreamProxyErrorCallback, canReplay, selectProtocol, serverEntries[0], nil, nil, false, 0, 0)
	if err != nil {
	...
	for i, serverEntry := range serverEntries {
		...
		if i%10 == 0 {
			...
			dialParams, err := MakeDialParameters(
				clientConfig, steeringIPCache, nil, canReplay, selectProtocol, serverEntry, nil, nil, false, 0, 0)
			if err != nil {
				t.Fatalf("MakeDialParameters failed: %s", err)
			}
