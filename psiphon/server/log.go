...
func (logger *TraceLogger) LogRawFieldsWithTimestamp(fields LogFields) {
	...
	if ShouldLogProtobuf() {
		for _, protoMsg := range LogFieldsToProtobuf(fields) {
			...
			serializedProto, err := proto.Marshal(protoMsg)
			if err != nil {
				logger.WithTrace().Errorf("failed to serialize protobuf message: %s", err.Error())
				continue
			}

			metricSocketWriter.WriteMessage(serializedProto)
		}
	}
...
var log *TraceLogger
var logFormat, logHostID, logHostProvider, logBuildRev string
...
func InitLogging(config *Config) (retErr error) {
	...
	initLogging.Do(func() {
		...
		if !slices.Contains([]string{"json", "protobuf", "both"}, config.LogFormat) {
			retErr = errors.Tracef("invalid log format: %s", config.LogFormat)
		}
